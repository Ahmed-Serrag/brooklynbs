class Endpoints {
  static String baseUrl =
      'https://srm-vbc7.onrender.com/api/m-app-users/check-credentials';
  static String token =
      "";

  static String forgetPw =
      'https://srm-vbc7.onrender.com/api/m-app-users/forget-passwords';

  static String request = 'https://task-com.onrender.com/api/mobile-requests';
  static String userUpdate =
      'https://task-com.onrender.com/api/mobile-user-updates';
  static String reqToken =
      '';
}

class PayMob {
  static String baseUrl = 'https://accept.paymob.com/';
  static String api_key =
      '';
  static String sk_key =
      'egy_sk_test_ebc24ab35db7779f4cd97327d937de666090c5495b4d978412a7419688761570';
  static String pk_key = 'egy_pk_test_IJSBa266eJAwwBHzlpxVWIoYxz5wEn2R';
}
